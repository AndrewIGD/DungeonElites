...
public class wizard : MonoBehaviour
{
    ...
    public void SetUpText()
    {
        try
        {
            for (int i = 0; i < player.attacks.Length; i++)
            {
                ...
                {
                    player.abilityDescs[i].abilityText.text = "Teleports <color=purple><b>" + (teleportDistance + player.originalSpeed) + "</b></color> units forward.";
                }
    ...
    public void Teleport()
    {
        ...
        {
            if (CheckGoodPos(myTransform.position - myTransform.up * (dist + player.originalSpeed)) == true||(player.inDungeon == false&&notInShops))
            {
                myTransform.position = myTransform.position - myTransform.up * (dist + player.originalSpeed);
    ...
    public bool CheckGoodPos(Vector2 pos)
    {
        Collider2D[] boxes = Physics2D.OverlapBoxAll(pos, new Vector2(0.01f, 0.01f), 0f);
        Collider2D[] boxes2 = Physics2D.OverlapBoxAll(myTransform.position, new Vector2(0.01f, 0.01f), 0f);
        bool ok = false;
        if(!notInShops)
        {
        ...
        else
        {
            ...
            for(int i=0;i<boxes.Length;i++)
                {
                    if (boxes[i].GetComponent<room>() != null)
                    {
                        ok = true;
                    }
                }
